...
延續上一篇，我們已經在 `hall` 建立一個聊天室物件 `mockChatroom` ，並在 `hall.component.html` 顯示其路由連結，點選該聊天室則導引到相關頁面。實際上大廳會存在許多聊天室物件，在介紹服務（service）之前，先嘗試在 `hall` 建立更多測試資料，確保聊天室顯示與連結均正常：
...
2. 在 `hall.component.ts` 加入多筆測試資料，並將其依序顯示在 `hall.component.html` 中。
  ...
  * 第 4 ~ 8 行大括號內則是用來呈現每個 `chatroom` 項目，我們建立路由與每個路由顯示各自的 `chatroom.name` ，經由點選導引到不同的聊天室頁面。
  * 第 9 ~ 10 行會在 `chatrooms()` 為空（或 `null` 或 `undefined` ）時顯示 No chatrooms 字樣。
...
使用 Angular CLI 即可建立一個服務，例如想要在既有的 `src/app` 下新增一個資料夾 `services` 並在其中建立服務 `ChatroomService` ，只要使用 `ng generate service services/chatroom` 或 `ng g s services/chatroom` 即可完成檔案建置。接著進行以下步驟：
